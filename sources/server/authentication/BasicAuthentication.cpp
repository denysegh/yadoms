...
namespace authentication
{
   ...
   void CBasicAuthentication::updateConfiguration()
   {
      try
      {
         ...
         m_isAuthenticationActive = basicAuthenConfiguration.getBoolWithDefault("active", false);
         m_currentAuthenticationUsername = basicAuthenConfiguration.getString("user");
         m_currentAuthenticationPassword = basicAuthenConfiguration.getString("password");
      }
      catch (std::exception& ex)
      {
